...
**Base:** `main` at `7c035fd` (the re-created repo's PR #4 merge, 2026-09-26 —
S28's post-merge documentation sync; it stays legal across the next merge because
...
| **U-59** | F:NF-02 | **Cancel / failure leaves truncated file over PRE-EXISTING output** — gifsicle writes direct to `-o <target>`; `cancelRun()` kills mid-write, failure branch never removes/restores; re-optimising existing `_opt.gif` + Cancel destroys last good result. `OutputPlan.h` declines temp+rename. **P0 data loss, same class as U-01.** | ✅ **EXEC** (S28) | ◐ PARTIAL (S28) — CLI/core half fixed with executed proof: `OutputVerify.h` gains the tmp+rename guard (`partial_output_path` / `redirect_output_operand` / `promote_partial` / `discard_partial`) and `main.cpp` redirects the single `-o <target>` operand to `<target>.gs-partial`, verifies the PARTIAL, and only then renames it onto the target, so the engine never touches the user's file; a cancel, a signal or a refusal discards the partial and the previous bytes survive. Proof: smoke 54 → **58/58** — the three new cases were run RED first against the unfixed build (`cmp=DIFFERS`, the pre-existing output destroyed) and are green after (byte-identical target for a refused run, an engine killed by SIGTERM, and a real SIGTERM cancel mid-write), plus the self-heal case proving the next run sweeps a partial a hard kill left behind; `test_output_verify.sh` 12 → **25 assertions** pins the helpers (redirect refuses a non-matching or doubled `-o`; promote really replaces the old bytes); unit 372/0 and the 79-case transport suite unchanged. REMAINING: the Qt `runCommand()` half — the GUI still runs the engine straight onto the target, so its Cancel path keeps the old behaviour until the same guard is wired there and the harness gets a cancel-with-preexisting case (needs Qt6; not compilable in the S28 sandbox). |
...
| P0-7 | **Cancel must not leave truncated file over pre-existing output — NEW F:NF-02 — fable 5.1 low — WINNER — P0 data loss.** gifsicle writes direct to `-o <target>`; Cancel kills mid-write, failure path never removes/restores. Re-optimising existing `_opt.gif` + Cancel destroys last good. Fix: run to `<target>.gs-partial`, verify GIF magic + non-empty, rename; on cancel/failure delete partial if pre-run did not exist, warn damaged if did. Snapshot pre-existence in `runCommand()`. Harness: cancel-with-preexisting keeps old bytes. **Same P0 class as U-01, requires tmp+rename guard.** **S28: CLI/core half IMPLEMENTED — the engine now writes `<target>.gs-partial` and the target is renamed onto only after verification, so a cancel/signal/refusal leaves the previous bytes intact (smoke 58/58, three cases run RED first; `test_output_verify.sh` 25 assertions). The Qt `runCommand()` half remains: the GUI still writes straight onto the target and its harness has no cancel-with-preexisting case.** | **U-59** | **F highest — WINNER** |
