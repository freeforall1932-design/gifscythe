...
**Status:** ◐ **PARTIAL (S8)** — the silent half is closed (both packagers now hard-require LICENSE + COPYING.gifsicle + COPYING.ms-pl, with negative tests); full Ms-PL text + first-party grant staged S18 (OD-09 = b). Still missing: Qt LGPL notices; register §5 `U-08`. Original report: ⬜ **OPEN** — confirmed by execution (C:U-08). Root had `LICENSE` (1133 B at the time) and `COPYING.gifsicle` but **no `COPYING`**, so the `package_system` copy branch never fired.
...
| **U-08** | A:GS-007 | **License set can ship incomplete, silently.** Root has `LICENSE` + `COPYING.gifsicle` but **no `COPYING`**; every license copy is `if [[ -f ]]`-guarded. | ✅ **EXEC**+SRC | ◐ PARTIAL (S8) — silent-skip closed: both packagers now hard-require LICENSE + COPYING.gifsicle + COPYING.ms-pl, with negative tests; full Ms-PL text + first-party grant staged S18 (OD-09 = b). Still missing: Qt LGPL notices |
...
| P1-26 | **Packaging fail-open paths — PARTIAL S17.** Both package types share fresh staging and a required-file manifest; explicit headless excludes GUI; portable Windows GUI requires a successful deployer and key Qt DLL/plugin entries. 32 Linux checks cover failures and real native CLI packages (30 + 1 per kind for the required COPYING.ms-pl case, S18; portable side executed, system side same shared code path; full green re-measured in CI). Actual Qt/Windows deployment, architecture and clean-machine proof remain. | **GS-204** | **A (S17 partial; platform handoff)** |
