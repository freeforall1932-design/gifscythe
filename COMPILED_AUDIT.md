...
**Status:** ✅ **FIXED (S30)** — register §5 `U-59`. GUI ordinary-output runs now use the shared verified-partial promotion lifecycle; Qt-enabled PR CI run 36227561080 passed Linux and Windows builds and both offscreen harnesses; after merge, main run 36227885126 passed as well. PR #5 merged with explicit owner approval as 957c143. See the U-59 register row below; the independent review remains a separate next-agent follow-up.
...
| **U-59** | F:NF-02 | **Cancel / failure leaves truncated file over PRE-EXISTING output** — gifsicle writes direct to `-o <target>`; `cancelRun()` kills mid-write, failure branch never removes/restores; re-optimising existing `_opt.gif` + Cancel destroys last good result. `OutputPlan.h` declines temp+rename. **P0 data loss, same class as U-01.** | ✅ **EXEC** (S28) | ✅ FIXED (S30) — CLI/core proof remains as recorded below. GUI Batch/Merge/Auto writes to the same-directory `<target>.gs-partial`, snapshots/verifies the partial, promotes only verified GIF output, and discards it on failure, failed start, verification/promotion failure, or cancel; Explode remains prefix-based. Offscreen T4/T8/T9 cover successful promotion, a fake engine that writes corrupt partial bytes then fails while preserving existing target bytes, and cancellation preserving existing bytes; failure/cancel assert no `.gs-partial` remains. Qt-enabled PR CI run **36227561080** and post-merge main run **36227885126** passed Linux and Windows builds and both GUI offscreen suites; PR #5 merged as `957c143` with explicit owner approval. Follow-up independent CMake/Qt review is separately queued for the next capable agent. |
...
| **U-81** | H:F-01 | **Explode frame verification ignores the stream-output and --info exemptions** — main.cpp gates the ordinary verifier on `!stream_output && !s.info` (documented contract) but the explode snapshot/verify blocks are ungated and `explode_prefix_for()` returns `output` verbatim: with `output = -` the CLI hunts files named "-.*", finds none, and downgrades an honest stdout run to rc=1 "wrote no frames"; info+explode hits the same false failure under advisory mode. | ✅ **EXEC** (S28) | ✅ FIXED (S28) — MEASURED first, and the intake's stated repro was wrong on BOTH halves: `gifsicle -e -o - in.gif` writes ZERO bytes to stdout and drops `in.gif.000..011` into the CWD (rc=0) — it is N-05's scatter class, not "an honest stdout run" — and `--info` + explode is refused by the ENGINE itself (`'--info' suppresses normal output`, rc=1), so no false frame failure existed there either. Fix: `verify_explode` now carries the same `!stream_output && !info` exemptions as `verify_file` (so the verifier can never add its own verdict to an exempt run), and explode + `output = -` gets N-05's treatment — a named refusal, rc=2, before any process starts (print mode still prints). Proof: smoke 59 → **61/61** with two new cases (rc=2 + a named reason + ZERO files left in the CWD; and the engine's own refusal reported without a "wrote no frames" overlay), unit 372/0 |
...
| **U-83** | H:F-07 | **Batch + explicit output + single input is neither refused nor pinned** — GS-201 closed no-output, U-74 closed N>1; with N=1 the planner classifies the merge shape, argv carries both -b and -o, and no smoke case pins what the engine actually does (an external live probe saw -b -o write to -o, source untouched — the gap is the missing pin, not known misbehavior). | ✅ **EXEC** (S28) | ✅ FIXED (S28) — pinned rather than refused, because the measured behaviour is sound: `mode = batch` + one input + an `output` key writes the `-o` target and leaves the SOURCE byte-identical (new smoke case asserts rc=0, a non-empty output and `cmp -s` on the source). Smoke 61/61. The engine-shape note (argv carries both `-b` and `-o`) is now recorded in the case itself, so a future engine that changes this fails the suite instead of drifting |
| **U-84** | H:F-08 | **/optimize discovers the engine before reading the request body** — an oversized upload to an engine-less server answers 503 (engine not found) instead of 413 (too large); /run reads the body first (its own comment says so), so identical clients get a different cap contract per endpoint. | ✅ **EXEC** (S28) | ✅ FIXED (S28) — `handleOptimize` now reads and admits the body BEFORE `findEngine()`, exactly like `handleRun`, so the two endpoints share one body-cap contract and a refused upload never takes an engine slot. Proof: a new engine-less server case in `body-limit.test.mjs` — `GS_ENGINE` pointing at a nonexistent path, oversized body → **413, not 503** — plus the parity case that `/run` and `/optimize` both answer 413 with no engine present. The suite's own header comment, which described the old ordering as current fact and explained the `GS_ENGINE=node` workaround for it, is corrected |
| **U-85** | H:F-09 · G:GN-11 | **A non-numeric PORT crashes the server with a raw RangeError stack** — `Number("abc")`=NaN and `listen(NaN)` throws ERR_SOCKET_BAD_PORT at module top level: no usage message, no exit-2 caller-error shape; self-hosting panels that set PORT hit this first, against the repo's own named-reason honesty rule. | ✅ **EXEC** (S28) | ✅ FIXED (S28) — PORT is validated once at startup: a decimal-integer shape test plus a 0..65535 range check, a named `ERROR: port must be a decimal integer 0..65535, got "..."` with a usage line on stderr, and **exit 2** (the CLI's caller-error convention). Proof: 4 new `server-bounds` cases (`abc`, `-1`, `99999`, `80.5`) each assert exit 2, the named reason, a usage line, and the ABSENCE of `RangeError`/`ERR_SOCKET_BAD_PORT`/any stack frame — 10/10 groups green |
| **U-86** | H:M-1/M-2/M-3 | **Three minor comment-vs-behavior mismatches** — the /run batch-collision message claims it prevents overwriting "the uploaded file of the same name" (uploads are renamed inN.gif; it prevents a planned output colliding with a user-facing upload name); expand_home's comment says "Reject bare ~ alone" but bare ~ expands to $HOME; run() resolves on timeout and again on close (first wins today — a trap for future close-time logic). | ✅ **EXEC** (S28) | ✅ FIXED (S28) — all three comment/message truths corrected. (1) The `/run` collision message claimed it protects "the uploaded file of the same name" while uploads are renamed `inN.gif`; it now says a planned output "collides with an upload name in this request", and the two transport assertions that pinned the old wording were re-pinned to the new one (not deleted). (2) `expand_home`'s "Reject bare ~ alone" comment now states what the code does: a bare `~` expands to `$HOME`, only `~user` is left alone. (3) `run()` could resolve twice (timeout then close) with the first winning only by accident; a `settleOnce` guard now makes the single resolution explicit, with the comment saying why |
...
| **U-92** | G:GN-10 | **Both web endpoints accept arbitrary bytes: no GIF-magic admission on uploads and forgiving base64** — /run checks only a non-empty name + uploadNameError; hasGifMagic exists but guards the explode OUTPUT only; Buffer.from(str,"base64") silently decodes truncated/foreign payloads; a non-GIF body surfaces as an engine stderr relay instead of a named 400 — GS-205's web twin (that row names Qt files only). | ✅ **EXEC** (S28) | ✅ FIXED (S28) — admission on BOTH endpoints, before engine discovery, slot and temp tree: `isStrictBase64()` (shape + canonical round trip, because `Buffer.from(s,"base64")` silently decodes truncated and foreign payloads) and the existing `hasGifMagic()` applied to the DECODED upload, not just to explode output; the decoded buffers are kept so nothing decodes twice. `/optimize` gets the same signature check on its body. A non-GIF upload is a named `400` that names the file, instead of the engine's stderr relay. Proof: 7 new `transport` cases (non-GIF, truncated header, empty, non-base64 text, non-canonical padding, a real GIF still admitted, and a launch probe proving refused uploads never reach the engine) — transport 79 → **86 cases** — plus 2 in `body-limit` |
| **U-93** | G:GN-12 | **Web transport shape is unbounded: stderr capture has no cap and is echoed in 422 bodies, /run inlines every output as base64 in one JSON response, and /favicon.ico 404s** — a chatty engine or a large batch inflates memory and wire (a response larger than the request that produced it); index.html carries no favicon and the allow-list has no entry. | ✅ **EXEC** (S28) | ✅ FIXED (S28) — three bounds. (1) Engine stderr capture is capped (`GS_MAX_STDERR`, default 16 KiB) and the cap is DISCLOSED: a truncated message ends with `[stderr truncated at N characters]`, so a client can tell a complete message from a cut one; the fixture had to write to stderr, not stdout, because the server discards stdout — measured, not assumed. (2) `/favicon.ico` answers `204` and `index.html` carries an empty data-URI icon, so a page load no longer logs a 404 against the allow-list. (3) The `/run` base64 output envelope is documented as a deliberate shape in `web/README.md` rather than left as an unstated growth risk. Proof: a new `server-bounds` group (a 400-line chatty FAILING engine under a 64-char cap → 422 with a bounded, marked stderr) — 10/10 groups — and `static-hygiene` still green |
| **U-94** | G:GN-13 | **No suite sweeps the settings space against the real engine: hand-written mirrors prove agreement, not correctness** — U-62, U-63 and N-05 were all found because BOTH mirrors encoded the same wrong rule and only gifsicle disagreed; the engine runs in test_engine/smoke, but nothing enumerates the settings space through it, so the class is still found by careful humans, one session at a time. | ✅ **EXEC** (S29: built CLI + bundled engine) | ✅ FIXED (S29) — `scripts/oracle_fuzz.mjs` runs a seeded, offline 64-case matrix (24-case `--quick` prefix): JS argv vs real C++ CLI parity, validator/strict-CLI agreement, each candidate against bundled gifsicle, accepted settings must produce verified GIF output, and any engine refusal must be warned/refused by product; committed `tests/oracle_fuzz_matrix.json` makes outcome drift reviewable. `verify_audit.sh` W7 + Linux CI run `--full`; pre-push runs `--quick`. Proof: 64/64 full, 24/24 quick, matrix comparison passes |
...
| P0-7 | **Cancel must not leave truncated file over pre-existing output — NEW F:NF-02 — fable 5.1 low — WINNER — P0 data loss.** gifsicle writes direct to `-o <target>`; Cancel kills mid-write, failure path never removes/restores. Re-optimising existing `_opt.gif` + Cancel destroys last good. Fix: run to `<target>.gs-partial`, verify GIF signature + non-empty, rename only on success, discard partial on cancel/failure. **Same P0 class as U-01.** **S28:** CLI/core guard implemented and RED-first tested (smoke 58/58; `test_output_verify.sh` 25 assertions). **S30 DONE:** GUI guard implemented; T4/T8/T9 cover promotion, partial-write failure and cancel preservation. Qt CI run **36227561080** passed Linux and Windows builds plus both offscreen GUI suites. Separate independent CMake/Qt review remains a next-agent task; no merge without owner approval. | **U-59** | **F highest — WINNER** |
...
| P2-18 | **Oracle-fuzz conformance gate — DONE S29.** `scripts/oracle_fuzz.mjs` (seeded, offline, zero-dep) enumerates 64 settings samples; `--quick` checks the 24-case boundary prefix for pre-push, `--full` checks the full committed matrix as verify_audit W7 + Linux CI. Every sample compares JS argv to the real C++ CLI; product validation is checked against strict-CLI refusal; accepted samples run against bundled gifsicle and require verified GIF output; any engine refusal must have a product warning/refusal. `tests/oracle_fuzz_matrix.json` commits the settings and measured outcomes so drift is reviewable. Executed: `--quick` 24/24 and `--full` 64/64, including engine-refusal, engine-accepts-but-product-warns, and verified-acceptance cases. | **U-94** | **G (S24 intake)** |
