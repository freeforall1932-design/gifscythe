...
- **NF-11 / U-68** — POST /run with a 50 MB GIF → 413 (not 400); the documented cap matches the real cap.
...
1. **Re-run the broad suite green** (the §17.1 validation order): `build.sh --all`, `smoke_cli.sh`, the offscreen `test_gui_offscreen`, the three `web/test/*.mjs`, `web/wasm/glue_harness.mjs`, and `verify_audit.sh`. Quote the **runtime** counters, never the `CHECK(` source-site counts (gate G9 enforces this).
