...
**Status:** ⬜ **OPEN** — confirmed by source read; register §5 `U-53`. Original report: ⬜ OPEN — U-33 fix incomplete.
...
**Status:** ◐ **PARTIAL (S21)** — register §5 `U-67`, which now carries the narrowed, measurement-backed wording. The finding text above is kept **verbatim as filed** for attribution; S21 measured each sub-claim against the real server before fixing anything, and only one of the three survived:

| Sub-claim as filed | S21 measurement (raw un-normalised HTTP against the live server) |
|---|---|
| "Every file under `web/` served" | ✅ **TRUE.** `/server.mjs` → 200, 26 KB; `/test/transport.test.mjs` → 200, 35 KB; `/run-paths.mjs`, `/validate.mjs`, `/output-verify.mjs`, `/README.md`, `/WEB_PLAN_TEMPLATE.md`, `/wasm/wasm.js`, `/wasm/index.html`, `/wasm/build_wasm.sh`, `/wasm/config.wasm.h` → all 200. |
| "raw prefix containment … would admit `/repo/web-anything`" | ⚠️ **OVERSTATED as written** — brittle hygiene, not a demonstrated reachable traversal. `new URL()` collapses dot-segments before `serveStatic()` sees them: `/../STATUS.md`, `/../../etc/hostname`, `/..%2fserver.mjs`, `/../working_code/gifscythe/VERSION.md` all → 404. `/../server.mjs` and `/%2e%2e/server.mjs` → 200 only because they normalise to `/server.mjs`, which is **inside** ROOT. Note the intake text itself already conceded "not exploitable today". The sibling-prefix concern (`/repo/web-x`) is real in principle but unreachable through the HTTP path. |
| "HEAD gets body" | ❌ **FALSE by measurement.** `HEAD /` and `HEAD /app.js` → 200 with `bodyLen=0`. Proven at the socket level: Node suppresses HEAD bodies itself (server wrote 5000 bytes, client received 0). A "headers-only for HEAD" fix changes nothing observable — so it is implemented as an **explicit, pinned contract** rather than a bug fix, to stop a future refactor regressing behaviour Node currently gives free. |

**Landed (S21):** four-entry allow-list (`/`, `/index.html`, `/style.css`, `/app.js`, `/command.mjs`) — the closed set the UI loads, verified from `index.html`'s only two asset references and `app.js`'s only import (`./command.mjs`, a leaf module); everything else under `web/` → 404; `assertContainedPath` replaces the raw prefix check as defence in depth; one `sendStatic()` path gives 200/403/404 a single HEAD contract with `Content-Length`. `web/wasm/` is experimental and not shippable (owner decision S21), so it is deliberately non-routable here — `web/wasm/README.md` already says to serve that page with any static server rooted at `web/`. **Executed proof:** `web/test/static-hygiene.test.mjs` 43/43, red→green (18 failures with the fix stashed), UI assets byte-exact with correct MIME, no engine required. **Remaining:** the engine-gated `transport.test.mjs` re-run and CI wiring. Original report: ⬜ **OPEN**.
...
| ID | Src | Finding | Verif | Status |
|----|-----|---------|-------|--------|
| **U-53** | E:NA-01 | **Position half-parse leaves has_position true with X,0** — `set_field()` sets `has_position=true` on first valid coordinate; `load_settings()` records saw_x/y before parsing success; valid x + invalid y emits `-p 12,0`. Regression of U-33 fix. | ✅ **SRC** | ⬜ OPEN |
...
| **U-60** | F:NF-03 | **CLI resolves `#0` frame selector to bogus path** — `GifsicleSettings.h` documents `#0` as legal input, man page defines it, but `resolve_path()` applied to every `s.inputs` turns `#0` into `/abs/#0` and fails. | ✅ **SRC** | ⬜ OPEN |
| **U-61** | F:NF-04 | **`output = -` treated as file → false failure rc=1** — man page `-o - means stdout`; CLI streams GIF to stdout correctly but `verify_output("-")` reports missing file and returns rc=1. Valid GIF + failure code. | ✅ **SRC** | ⬜ OPEN |
| **U-62** | F:NF-05 | **Validate.h refuses crop W/H 0, engine allows 0=extend to edge** — man `--crop x1,y1+WxH`: width/height can be zero or negative, zero=to edge. Validator rejects 0, unsigned makes negative unrepresentable. | ✅ **SRC** | ⬜ OPEN |
...
| **U-64** | F:NF-07 | **`/run` and `/optimize` accept `info:true` then misleading 422** — CLI exempts info from output verification, web does not; `info -o file` writes TEXT, web `verifyOutput` sees no GIF magic and blames engine with exitCode 0. | ✅ **SRC** | ⬜ OPEN |
...
| **U-67** | F:NF-10 | **`serveStatic()` served the whole `web/` tree — server source, test suite and docs were world-readable** (narrowed by S21 measurement; two of the three original sub-claims did not survive it — see §2F F-10). Confirmed true: with no allow-list, `GET /server.mjs` returned 26 KB of server source (disclosing the loopback bind and `GS_ENGINE` handling, which matters because `GS_WEB_HOST=0.0.0.0` is a documented opt-in), and `/test/transport.test.mjs` (35 KB), `/run-paths.mjs`, `/validate.mjs`, `/output-verify.mjs`, `/README.md`, `/WEB_PLAN_TEMPLATE.md` and `/wasm/*` were all served. Overstated as filed: the `file.startsWith(ROOT)` prefix check is brittle *hygiene*, not a demonstrated reachable traversal — `new URL()` normalises dot-segments before `serveStatic` sees them, so `/../STATUS.md`, `/../../etc/hostname` and `/../working_code/gifscythe/VERSION.md` already returned 404; `/../server.mjs` returned 200 only because it normalises to `/server.mjs`, which is inside ROOT. False as filed: "HEAD returns a body" does not reproduce — Node suppresses HEAD bodies itself (measured: server wrote 5000 bytes, client received 0). | ✅ **EXEC** (S21) | ◐ PARTIAL (S21) — REMAINING: the engine-gated `transport.test.mjs` no-regression re-run (its only static dependency is the readiness `GET /`, which is preserved and byte-exact verified) and CI wiring of the new test. LANDED + EXEC-proven: `serveStatic()` now serves a four-entry allow-list (`/`, `/index.html`, `/style.css`, `/app.js`, `/command.mjs` — the closed set the UI loads; `web/wasm/` is experimental and not shippable per owner decision S21, so it is deliberately non-routable), everything else under `web/` is 404, the raw prefix check is replaced by the same `assertContainedPath` resolved-path containment `run-paths.mjs` enforces on engine outputs (defence in depth), and one `sendStatic()` path gives 200/403/404 a single explicit HEAD contract with `Content-Length`. Proof: `web/test/static-hygiene.test.mjs` 43/43, red→green (18 failures stashed), UI assets byte-exact with correct MIME, no engine needed. |
...
| P1-33 | **Fix position half-parse — NEW E:NA-01 — gpt 5.6 sol xhigh.** Stop mutating `has_position` in `set_field()`; parse into optional temps, enable only when both present and both conversions succeeded; one pair-level warning; keep --strict rc=3; add unit test for `x=12,y=nope` → no `-p`. Regression of U-33. | **U-53** | **E highest** |
...
| P1-39 | **Frame selectors #0 and stdout - — NEW F:NF-03/04 — fable 5.1 low — WINNER.** Skip `resolve_path()` when input starts with `#` or equals `-`; exclude from `plan_outputs()` input keys; normalise `output=="-"` to streaming contract before planning/verification; smoke tests for `#0` and `-`. | **U-60, U-61** | **F — WINNER** |
| P1-40 | **Crop 0 and loop once — NEW F:NF-05/06 — fable 5.1 low — WINNER.** Allow crop W/H 0 = extend to edge per man, consider int for negative; add `loopcount=-2 → --no-loopcount`; GUI Play once, web `once`; update JS mirror + parity fixtures. | **U-62, U-63** | **F — WINNER** |
| P1-41 | **Web info:true and engine beside symlink — NEW F:NF-07/08/09 — fable 5.1 low — WINNER.** Reject `info` at web validation (400) or return text/plain; resolve real exe via `/proc/self/exe` / `_NSGetExecutablePath` / `GetModuleFileNameW` + `canonical()`; unify version policy: release/current symlink or newest fallback in EngineLocator, log chosen dir. | **U-64, U-65, U-66** | **F — WINNER** |
...
| P2-16 | **Web static hygiene and 413 mapping — NEW F:NF-10/11/12 — fable 5.1 low — WINNER.** Reuse `assertContainedPath(ROOT, file)` in `serveStatic()`, allow-list served files, headers-only for HEAD; typed 413 from `readBody()` and map; document effective 48MB cap or raise MAX_BODY; `revokeResults()`/hide before every run or on failure to avoid stale After. **U-67 (serveStatic allow-list + `assertContainedPath` containment + explicit HEAD contract) and U-68 (the 413 mapping + cap documentation) are both PARTIAL S21** — `web/test/static-hygiene.test.mjs` 43/43 and `web/test/body-limit.test.mjs` 8/8, each red→green with no engine required. Remaining for both: the engine-gated `transport.test.mjs` no-regression re-run and CI wiring. **U-69 (stale After image, `web/app.js`) untouched** — it is browser-DOM behaviour this sandbox cannot exercise. | **U-67, U-68, U-69** | **F — WINNER** |
...
11. **2026-09-15 — S21 first code task: U-68 / NF-11 oversized body → 413 (PARTIAL).** Chosen as the single highest-confidence task this sandbox can actually prove: it has **no compiler** (node v20.20.2 / python3 / git only), so no C++/CLI/Qt/Windows/wasm row is buildable and the existing web suites cannot run (command/validate spawn the C++ CLI; transport needs a discoverable engine; glue needs emcc). U-68 is HTTP-transport-only — the desktop has no HTTP server, so there is **no C++ parity mirror to diverge from** — and `/run` reads the body before `findEngine()`, so the 413 is provable with no engine.
   ...
   - **§5 U-68 → ◐ PARTIAL (S21)** (was ⬜ OPEN); §2F F-11 narrative and §6 P2-16 annotated to match (gate S5); §17.2 NF-11 regression case marked implemented. **STATUS.md re-emitted** via `check_docs.sh --emit` (U-68 OPEN→PARTIAL, so the generated counts moved with it — the tally is quoted only in `STATUS.md` and the handoff's current-register line, never in a changelog entry, because gate S2 would go stale the moment the next row moves); `SESSION_HANDOFF.md` tally quote updated (gate S2) and an S21 section added; `IMPROVEMENT_LOG.md` S21 entry added (gate G11 — a non-doc file changed). REMAINING for DONE: the engine-gated `transport.test.mjs` no-regression re-run, and the cap value is documented but deliberately unchanged. U-67/U-69 (the rest of P2-16) untouched.
12. **2026-09-15 — S21 second code task: U-67 / NF-10 `serveStatic` hygiene (PARTIAL), after measuring the finding first.**
   - **Measured before fixing.** Probed the live server with raw un-normalised HTTP paths (`http.request`, because `fetch()` normalises the URL before it goes on the wire). Of NF-10's three sub-claims only one reproduced: **"every file under `web/` served" is TRUE** (`/server.mjs` → 200 / 26 KB, `/test/transport.test.mjs` → 200 / 35 KB, plus `run-paths.mjs`, `validate.mjs`, `output-verify.mjs`, `README.md`, `WEB_PLAN_TEMPLATE.md`, `wasm/*`); **"raw prefix containment" is OVERSTATED** (`new URL()` collapses dot-segments first, so `/../STATUS.md`, `/../../etc/hostname`, `/../working_code/gifscythe/VERSION.md` already 404 — `/../server.mjs` returned 200 only because it normalises to `/server.mjs`, inside ROOT; the intake text itself conceded "not exploitable today"); **"HEAD returns a body" is FALSE** (measured `bodyLen=0`; Node suppresses HEAD bodies — proven at socket level: server wrote 5000 bytes, client received 0).
   - **Owner decisions taken before implementing** (both recorded in §5/§2F): allow-list is **UI-only**, because this Node server is the supported shipped web surface and `web/wasm/` is experimental / not shippable; and **U-67 is corrected to match the measurements** rather than preserving the inaccurate three-part wording — §2F keeps the intake text verbatim for attribution and adds a per-claim measurement table, while §5 states only what is true.
   - `web/server.mjs`: `STATIC_FILES` allow-list of the four files the UI actually loads (`/`→`index.html`, `/index.html`, `/style.css`, `/app.js`, `/command.mjs`), verified as a closed set from `index.html`'s only two asset references and `app.js`'s only import (`./command.mjs`, a leaf module). Everything else under `web/` → 404. The raw `file.startsWith(ROOT)` check is replaced by `assertContainedPath(ROOT, …)` — the same resolved-path containment `run-paths.mjs` enforces on every engine output target and `transport.test.mjs` already unit-tests — as defence in depth, so a future allow-list entry cannot reintroduce a prefix escape. One `sendStatic()` path gives 200/403/404 a single explicit HEAD contract with `Content-Length`. `serveStatic()` gained a `req` parameter (call site updated); `normalize` is no longer imported.
   - **Executed proof:** new `web/test/static-hygiene.test.mjs` **43/43, red→green** — stashing the fix reproduces 18 failures (every over-exposed path, the `server.mjs` disclosure check, and the two normalise-to-inside-ROOT traversals) while the UI byte-exact, HEAD-contract and above-ROOT-traversal cases already passed, so the test isolates the real defect instead of passing vacuously. `body-limit.test.mjs` re-run: still 8/8 (no regression). No engine needed — `serveStatic` never touches `findEngine()`.
   - **§5 U-67 → ◐ PARTIAL (S21)** (was ⬜ OPEN) with the narrowed wording; §2F F-10 keeps the filed text and adds the measurement table; §6 P2-16 and §17.2 NF-10 annotated; §19 gained a "measure before you fix" mandate and lists both new tests. **STATUS.md re-emitted** (U-67 OPEN→PARTIAL; counts live in `STATUS.md` only). `web/wasm/README.md` "Serve the page" now states that `web/server.mjs` deliberately does not route `wasm/`, and why (that page imports `../validate.mjs`, which the shipped UI does not — verified by serving `web/` with `python3 -m http.server`). REMAINING for DONE: the engine-gated `transport.test.mjs` re-run (its only static dependency is the readiness `GET /`, preserved and byte-exact verified) and CI wiring. **U-69 untouched** — it is browser-DOM behaviour this sandbox cannot exercise.
...
> GPT 6 Astra Medium, 7b91 NA-01..05 gpt 5.6 sol xhigh, 729d NF-01..19 fable 5.1 low WINNER) into one 76-finding register. **The current roll-up of every one of them —
...
### 14.4 CI-robustness observations made while diagnosing S21 (logged, not registered)

Logged here per the owner's standing instruction — *"doesn't add finding thats actually real if youre unsure put it at the bottom of unverified finding logging it instead of not adding it at all."* Both are **observed facts with run ids**, not speculation, but neither is a product defect, so neither gets a `U-nn` row. They are CI-cost / CI-trust items for whoever next touches `build.yml` (which needs `workflows` scope — see `docs/ci/PENDING_WORKFLOW_CHANGE.md`).

1. **The Windows `Install Qt6 (aqt) + MinGW toolchain` step has no retry and flakes.** Observed 2026-09-15 on PR #27: run `34995165855` (`pull_request`, head `15ffa1d`) failed that step with **exit code 254** and skipped steps 4–11 — so no build, test, doc gate or packaging step ever ran. The **identical** head passed all three jobs on run `34995156260` (`push`), and a `rerun-failed-jobs` of the failed run went green on attempt 2 with no code change. The step is `pip install aqtinstall` then `aqt install-qt windows desktop 6.7.3 win64_mingw` under `set -euo pipefail`: a pure network download from Qt's mirrors with one shot at it. *Consequence:* a red Windows job here can mean "the mirror hiccuped", not "the code broke" — which is exactly the ambiguity that makes a red CI get ignored. *Cheap mitigation:* retry the `aqt install-qt` call 2–3× with a short backoff, or pin a Qt mirror/cache. *Do not* weaken `set -euo pipefail` to get past it.
2. **Every push to a PR branch runs the whole workflow twice.** `build.yml` triggers on bare `on: push:` **and** `on: pull_request:` with no branch or path filters, so a branch with an open PR pays for two full linux + windows + csharp-spike runs per commit (visible above: two runs for the one head `15ffa1d`). That doubles the exposure to flake #1 and doubles the minutes. *Cheap mitigation:* scope `on: push:` to `branches: [main]` and leave `pull_request:` for branch work — the usual pattern. This is a cost/hygiene observation, not a correctness bug; nothing here claims a gate is weaker than it is.

**Not** claimed: that either observation caused any past red on `main`. GS-208 (the S14 release-red) was a documentation-gate failure, a different cause.
...
- **NF-10 / U-67** — `serveStatic` rejects `/../` traversal; HEAD returns no body; server source/tests are not served. **IMPLEMENTED S21 (PARTIAL), narrowed by measurement:** `web/test/static-hygiene.test.mjs` 43/43 red→green proves the allow-list (server source, tests and docs all 404), the four UI assets byte-exact with correct MIME, raw-path traversals never yielding a body, and the HEAD contract. Of the three sub-claims only "serves source/tests" reproduced: the traversal was latent (shielded by `new URL()` normalisation, as the intake itself conceded) and "HEAD returns a body" is **false** — Node suppresses HEAD bodies, so that part is pinned as a contract, not fixed as a bug. See §2F F-10 for the per-claim measurement table.
...
1. **Re-run the broad suite green** (the §17.1 validation order): `build.sh --all`, `smoke_cli.sh`, the offscreen `test_gui_offscreen`, the three `web/test/*.mjs` **plus the two node-only tests S21 added — `web/test/body-limit.test.mjs` (U-68) and `web/test/static-hygiene.test.mjs` (U-67), neither of which needs an engine**, `web/wasm/glue_harness.mjs`, and `verify_audit.sh`. Quote the **runtime** counters, never the `CHECK(` source-site counts (gate G9 enforces this). *Follow-up owed:* both new tests are repo-resident and proven here but are **not yet wired into CI** — `build.yml` lists the web tests explicitly and needs `workflows` scope (the same block as `docs/ci/PENDING_WORKFLOW_CHANGE.md`, and it has a byte-identical twin copy), and `verify_audit.sh` would need a `DOC_GATE_CHECKS` bump for the new W-gates (gate G6). Wire them deliberately; do not let them rot as local-only tests (that is the U-18/GS-201 "green CI enforces nothing" failure mode).
...
5. **Measure before you fix — S21 found two of NF-10's three sub-claims were not real.** "HEAD returns a body" is false (Node suppresses HEAD bodies) and the traversal was latent, not reachable. Both would have produced confident-looking "fixes" for non-bugs. So: reproduce each sub-claim against the running code *first*, mark the row with what actually reproduced (§2F F-10 has the per-claim table), and fix only that. A narrowed finding closed is worth more than a three-part finding "closed" on the strength of prose.
6. **Re-confirm the doc gate is green** (`check_docs.sh` 0 failed, `sweep_stale.sh` 0 failed) after every edit — the consolidation itself must not become the next GS-208 / N-01 doc-drift gate failure.
