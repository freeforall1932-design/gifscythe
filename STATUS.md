...
**Counts (generated - do not edit by hand):** 90 DONE · 8 PARTIAL · 48 OPEN · 0 UNTRIAGED · 146 total
...
| U-53 | Position half-parse leaves has_position true with X,0 — `set_field()` sets `has_position=true` on first valid coordinate; `load_settings()` records saw_x/y before parsing success; valid x + invalid y emits `-p 12,0`. | OPEN | - | not started; scoped as P1-33 in COMPILED_AUDIT.md §6 | P1-33: Fix position half-parse — NEW E:NA-01 — gpt 5.6 sol xhigh. |
...
| U-60 | CLI resolves `#0` frame selector to bogus path — `GifsicleSettings.h` documents `#0` as legal input, man page defines it, but... | OPEN | - | not started; scoped as P1-39 in COMPILED_AUDIT.md §6 | P1-39: Frame selectors #0 and stdout - — NEW F:NF-03/04 — fable 5.1 low — WINNER. |
| U-61 | `output = -` treated as file → false failure rc=1 — man page `-o - means stdout`; CLI streams GIF to stdout correctly but `verify_output("-")` reports missing file and returns rc=1. | OPEN | - | not started; scoped as P1-39 in COMPILED_AUDIT.md §6 | P1-39: Frame selectors #0 and stdout - — NEW F:NF-03/04 — fable 5.1 low — WINNER. |
| U-62 | Validate.h refuses crop W/H 0, engine allows 0=extend to edge — man `--crop x1,y1+WxH`: width/height can be zero or negative, zero=to edge. | OPEN | - | not started; scoped as P1-40 in COMPILED_AUDIT.md §6 | P1-40: Crop 0 and loop once — NEW F:NF-05/06 — fable 5.1 low — WINNER. |
| U-63 | `--no-loopcount` unrepresentable — engine has unchanged / forever (`0`) / N / OFF (`--no-loopcount` play once). | OPEN | - | not started; scoped as P1-40 in COMPILED_AUDIT.md §6 | P1-40: Crop 0 and loop once — NEW F:NF-05/06 — fable 5.1 low — WINNER. |
| U-64 | `/run` and `/optimize` accept `info:true` then misleading 422 — CLI exempts info from output verification, web does not; `info... | OPEN | - | not started; scoped as P1-41 in COMPILED_AUDIT.md §6 | P1-41: Web info:true and engine beside symlink — NEW F:NF-07/08/09 — fable 5.1 low — WINNER. |
