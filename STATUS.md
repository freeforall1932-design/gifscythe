...
| U-53 | Batch continuation re-reads LIVE settings — targets planned once (`batchTargets_` U-01) but `currentSettings()` called live for... | OPEN | - | not started; scoped as P1-38 in COMPILED_AUDIT.md §6 | P1-38: Snapshot batch settings — NEW E:NF-01 — gpt 5.6 sol xhigh. |
| U-54 | Cancel / failure leaves truncated file over PRE-EXISTING output — gifsicle writes direct to `-o <target>`; `cancelRun()` kills mid-write, failure branch never removes/restores; re-optimising existing `_opt.gif` + Cancel destroys last good result. | OPEN | - | not started; scoped as P0-7 in COMPILED_AUDIT.md §6 | P0-7: Cancel must not leave truncated file over pre-existing output — NEW E:NF-02 — gpt 5.6 sol xhigh. |
| U-55 | CLI resolves `#0` frame selector to bogus path — `GifsicleSettings.h` documents `#0` as legal input, man page defines it, but... | OPEN | - | not started; scoped as P1-39 in COMPILED_AUDIT.md §6 | P1-39: Frame selectors #0 and stdout - — NEW E:NF-03/04 — gpt 5.6. |
| U-56 | `output = -` treated as file → false failure rc=1 — man page `-o - means stdout`; CLI streams GIF to stdout correctly but `verify_output("-")` reports missing file and returns rc=1. | OPEN | - | not started; scoped as P1-39 in COMPILED_AUDIT.md §6 | P1-39: Frame selectors #0 and stdout - — NEW E:NF-03/04 — gpt 5.6. |
| U-57 | Validate.h refuses crop W/H 0, engine allows 0=extend to edge — man `--crop x1,y1+WxH`: width/height can be zero or negative, zero=to edge. | OPEN | - | not started; scoped as P1-40 in COMPILED_AUDIT.md §6 | P1-40: Crop 0 and loop once — NEW E:NF-05/06 — gpt 5.6. |
| U-58 | `--no-loopcount` unrepresentable — engine has unchanged / forever (`0`) / N / OFF (`--no-loopcount` play once). | OPEN | - | not started; scoped as P1-40 in COMPILED_AUDIT.md §6 | P1-40: Crop 0 and loop once — NEW E:NF-05/06 — gpt 5.6. |
| U-59 | `/run` and `/optimize` accept `info:true` then misleading 422 — CLI exempts info from output verification, web does not; `info... | OPEN | - | not started; scoped as P1-41 in COMPILED_AUDIT.md §6 | P1-41: Web info:true and engine beside symlink — NEW E:NF-07/08/09 — gpt 5.6. |
| U-60 | CLI symlink/PATH loses engine-beside-executable discovery — `exe_path_of(argv0)` keeps symlink, bare name only resolved if... | OPEN | - | not started; scoped as P1-41 in COMPILED_AUDIT.md §6 | P1-41: Web info:true and engine beside symlink — NEW E:NF-07/08/09 — gpt 5.6. |
| U-61 | Desktop pinned to GS_VERSION while web picks newest — CLI/GUI look in `release/<GS_VERSION>/`, web `findEngine()` picks newest numeric dir; VERSION bump breaks CLI/GUI while web still works. | OPEN | - | not started; scoped as P1-41 in COMPILED_AUDIT.md §6 | P1-41: Web info:true and engine beside symlink — NEW E:NF-07/08/09 — gpt 5.6. |
| U-62 | serveStatic raw prefix containment, serves source/tests, HEAD body — `file.startsWith(ROOT)` hygiene; every file under web/ served; HEAD returns body. | OPEN | - | not started; scoped as P2-16 in COMPILED_AUDIT.md §6 | P2-16: Web static hygiene and 413 mapping — NEW E:NF-10/11/12 — gpt 5.6. |
| U-63 | Oversized bodies 400 not 413, /run cap ~48MB not 64MB — `readBody` rejects generic Error, `handleRun` maps to bad JSON;... | OPEN | - | not started; scoped as P2-16 in COMPILED_AUDIT.md §6 | P2-16: Web static hygiene and 413 mapping — NEW E:NF-10/11/12 — gpt 5.6. |
| U-64 | After failed run previous After stays under Failed status — `revokeResults()`/hide only on success and queue change; setting... | OPEN | - | not started; scoped as P2-16 in COMPILED_AUDIT.md §6 | P2-16: Web static hygiene and 413 mapping — NEW E:NF-10/11/12 — gpt 5.6. |
| U-65 | Preview engine check bypasses UTF-8 boundary — `ensureEngine()` wraps `u8path_compat()`, `startPreview()` passes... | OPEN | - | not started; scoped as P1-42 in COMPILED_AUDIT.md §6 | P1-42: Preview UTF-8 boundary and cancelling lifetime — NEW E:NF-13/15 — gpt 5.6. |
| U-66 | Windows exit masked `&0xff` collapses NTSTATUS crash to success — `ProcessRunner.h` Windows `code &0xff`; crash NTSTATUS like... | OPEN | - | not started; scoped as P2-17 in COMPILED_AUDIT.md §6 | P2-17: Windows exit code and NTSTATUS — NEW E:NF-14 — gpt 5.6. |
| U-67 | cancelling_ cleared after 3s wait → spurious failure dialog after Cancelled — `cancelRun()` resets flag right after `waitForFinished(3000)`; if engine dies later, `finished()` arrives with `cancelling_==false` and shows error after Cancelled. | OPEN | - | not started; scoped as P1-42 in COMPILED_AUDIT.md §6 | P1-42: Preview UTF-8 boundary and cancelling lifetime — NEW E:NF-13/15 — gpt 5.6. |
| U-68 | resolve_path CWD fallback contradicts CWD-independent contract — relative input missing next to conf but exists in CWD picked... | OPEN | - | not started; scoped as P1-43 in COMPILED_AUDIT.md §6 | P1-43: CLI resolve_path and batch+output semantics — NEW E:NF-16/17/19 — gpt 5.6. |
| U-69 | Batch + output + N>1 passes planner in merge shape, engine semantics undocumented — GS-201 refuses Batch WITHOUT output; WITH... | OPEN | - | not started; scoped as P1-43 in COMPILED_AUDIT.md §6 | P1-43: CLI resolve_path and batch+output semantics — NEW E:NF-16/17/19 — gpt 5.6. |
| U-70 | `-E` exposed but `--name` not, several engine options absent — without `--name` `-E` vs `-e` differs only if input already... | OPEN | - | not started; scoped as P3-12 in COMPILED_AUDIT.md §6 | P3-12: -E without --name and prefix docs — NEW E:NF-18 — gpt 5.6. |
| U-71 | Explode default prefix differs per surface — GUI: `<dir>/<stem>_frame`, CLI: `<basename>.NNN` in CWD when output cleared;... | OPEN | - | not started; scoped as P1-43 in COMPILED_AUDIT.md §6 | P1-43: CLI resolve_path and batch+output semantics — NEW E:NF-16/17/19 — gpt 5.6. |
| U-72 | Position half-parse leaves has_position true with X,0 — `set_field()` sets `has_position=true` on first valid coordinate; `load_settings()` records saw_x/y before parsing success; valid x + invalid y emits `-p 12,0`. | OPEN | - | not started; scoped as P1-33 in COMPILED_AUDIT.md §6 | P1-33: Fix position half-parse — NEW F:NA-01 — fable 5.1 low — WINNER. |
| U-73 | Web settings change does not invalidate in-flight run — `requestGen` advances on queue change but not on settings input; controls editable while `fetch('/run')` pending; old response accepted while UI shows new settings. | OPEN | - | not started; scoped as P1-34 in COMPILED_AUDIT.md §6 | P1-34: Invalidate web run on settings change — NEW F:NA-02 — WINNER. |
| U-74 | Windows path_key folds ASCII only — `OutputPlan.h` lowercases UTF-8 bytes with `tolower` under `_WIN32`; non-ASCII case... | OPEN | - | not started; scoped as P1-35 in COMPILED_AUDIT.md §6 | P1-35: Windows path_key Unicode case fold — NEW F:NA-03 — WINNER. |
| U-75 | Desktop sanitization misses superscript COM/LPT aliases — `is_windows_reserved_device_name()` checks ASCII digit only; Windows reserves `COM¹²³` / `LPT¹²³` (U+00B9/00B2/00B3). | OPEN | - | not started; scoped as P1-36 in COMPILED_AUDIT.md §6 | P1-36: Superscript COM/LPT aliases — NEW F:NA-04 — WINNER. |
| U-76 | WASM singleton reuses stale /out.gif — `wasm.js` does not unlink/snapshot `/out.gif` before `callMain()`; after one success, exit-zero/no-write run reads previous GIF and reports success for wrong input. | OPEN | - | not started; scoped as P1-37 in COMPILED_AUDIT.md §6 | P1-37: WASM stale /out.gif — NEW F:NA-05 — WINNER. |
<!-- END GENERATED -->
...
Triage scoped them; it did not fix them. **S16 closed `GS-201` (P0-5); S17 closed `GS-202` (P0-6), `DS-13` (P1-32) and `GS-207` (P1-29)** with executed proof; S17 also closed `DS-11` (P2-14). `GS-203`, `GS-204` and `GS-210` are PARTIAL with explicit handoffs; ten intake findings remain OPEN.
...
| DS-06 | `threads <= 0` emits a bare `-j`, so the `-1` unset sentinel now means 8 threads instead of the engine default; no way to emit no flag | OPEN | S15 | `src/core/GifsicleCommand.h:226`, `src/core/GifsicleSettings.h:75`; upstream `gifsicle.c:38-39,1893`; unit test pins it (intake B.1) | **P0-2** — Tri-state: `<0` emits nothing, `0` bare `-j`, `>0` `-jN`; update unit test 28 and the settings comment |
| DS-07 | GUI Threads spinner spans 0..64 and always writes a value; the no-flag/unchanged state is unrepresentable | OPEN | S15 | `src/qtui/SettingsPanel.cpp:349-352,594` (intake B.2) | **P1-30** — Map the spinner minimum to -1 'Unchanged' (or document GUI-always-explicit); must agree with DS-06 |
| DS-08 | Non-strict print mode returns 0 even when validation warned, so scripts cannot tell valid from warned | OPEN | S15 | `src/cli/main.cpp:274-277` vs `--strict` at `:216-222` (intake B.3) | **P3-5** — Document the advisory contract; optional greppable warning summary line |
| DS-09 | `threads < -1` is accepted with no warning and re-interpreted as auto | OPEN | S15 | `src/core/SettingsIO.h:202-204`; no `threads` rule in `src/core/Validate.h` (intake B.4) | **P1-31** — Warn on `threads < -1` in C++ and the JS mirror; unit assertion for -7 |
