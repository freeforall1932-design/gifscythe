...
**Last regenerated:** S18 · 2026-09-14 · by scripts/check_docs.sh --emit
...
| U-08 | License set can ship incomplete, silently. | PARTIAL | S8 | silent-skip closed: both packagers now hard-require LICENSE + COPYING.gifsicle + COPYING.ms-pl, with negative tests; full Ms-PL text + first-party ... | close the gap named in Proof / Blocker |
...
| W-08 | Root LICENSE / COPYING.ms-pl / COPYING.gifsicle / .gitignore / .gitattributes | DONE | S18 | CI "Assert package manifest" step requires LICENSE + COPYING.gifsicle + COPYING.ms-pl in the package | - |
...
| GS-204 | Fail-closed, manifest-driven portable/system packaging | PARTIAL | S18 | Shared fresh staging + required non-empty files, explicit headless/Windows targets; 32 Linux checks (30 + 2 required-file cases S18) include both packagers, real native CLI artifacts and deployer fixtures. Actual Qt deployment/runtime and binary architecture checks remain unverified | **P1-26** — Real Windows deployment and clean-machine smoke; retain U-08/U-09 blockers; see `docs/planning/SEQUENTIAL_WORK_HANDOFF.md` |
