...
**Counts (generated - do not edit by hand):** 80 DONE · 5 PARTIAL · 35 OPEN · 1 UNTRIAGED · 121 total
**Last regenerated:** S15 · 2026-09-13 · by scripts/check_docs.sh --emit
...
| N-07 | Sweep **S4** detects only 5 hardcoded retired phrases (`demo only`, `not the product path`, `is a demo`, `demo/parity harness`, `not the product`), so it cannot catch a current-state doc asserting findings "stay `UNTRIAGED`" after a triage empties the register | UNTRIAGED | S15 | Measured: `sweep_stale.sh`'s `RETIRED=` is 5 literals. The S15 triage left 4 live claims of that shape (`SESSION_HANDOFF.md:121`, `WORKLIST.md:411`, `docs/planning/OWNER_DECISIONS.md:53`, `web/WEB_PLAN_TEMPLATE.md:98`) and the sweep reported `5 passed, 0 failed` both before and after they were corrected by hand | Add a measurable rule — a doc naming a count of `UNTRIAGED` findings that disagrees with the register's generated counts line — or restate S4's scope as a fixed phrase list rather than a general reversal detector |
...
None of these has been remediated: triage scoped them, it did not fix them.

| ID | Item | State | Session | Proof / Blocker | Next action |
|----|-------|-------|---------|-----------------|-------------|
| GS-201 | CLI **Batch** maps to the engine's in-place `-b`; with no `output` key the output planner is skipped, so `--run` can rewrite the source GIFs | OPEN | S15 | `working_code/gifscythe/src/core/GifsicleCommand.h:93` + `working_code/gifscythe/src/cli/main.cpp:284`; upstream `reference_code/gifsicle/gifsicle.1:190`; destructive repro NOT executed (intake A.1) | **P0-5** — P0 candidate: refuse `--run` with Batch and no output (exit 2) until a per-file Auto orchestrator exists (code change, owner OK) |
