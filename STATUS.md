...
**Counts (generated - do not edit by hand):** 96 DONE · 8 PARTIAL · 42 OPEN · 0 UNTRIAGED · 146 total
**Last regenerated:** S22 · 2026-09-16 · by scripts/check_docs.sh --emit
...
| U-54 | Web settings change does not invalidate in-flight run — `requestGen` advances on queue change but not on settings input; controls editable while `fetch('/run')` pending; old response accepted while UI shows new settings. | OPEN | - | not started; scoped as P1-34 in COMPILED_AUDIT.md §6 | P1-34: Invalidate web run on settings change — NEW E:NA-02 — gpt 5.6. |
...
| U-56 | Desktop sanitization misses superscript COM/LPT aliases — `is_windows_reserved_device_name()` checks ASCII digit only; Windows reserves `COM¹²³` / `LPT¹²³` (U+00B9/00B2/00B3). | OPEN | - | not started; scoped as P1-36 in COMPILED_AUDIT.md §6 | P1-36: Superscript COM/LPT aliases — NEW E:NA-04 — gpt 5.6. |
...
| U-63 | `--no-loopcount` unrepresentable — engine has unchanged / forever (`0`) / N / OFF (`--no-loopcount` play once). | OPEN | - | not started; scoped as P1-40 in COMPILED_AUDIT.md §6 | P1-40: Crop 0 and loop once — PARTIAL S22. |
...
| U-65 | CLI symlink/PATH loses engine-beside-executable discovery — `exe_path_of(argv0)` keeps symlink, bare name only resolved if... | OPEN | - | not started; scoped as P1-41 in COMPILED_AUDIT.md §6 | P1-41: Web info:true and engine beside symlink — PARTIAL S22. |
| U-66 | Desktop pinned to GS_VERSION while web picks newest — CLI/GUI look in `release/<GS_VERSION>/`, web `findEngine()` picks newest numeric dir; VERSION bump breaks CLI/GUI while web still works. | OPEN | - | not started; scoped as P1-41 in COMPILED_AUDIT.md §6 | P1-41: Web info:true and engine beside symlink — PARTIAL S22. |
| U-67 | serveStatic raw prefix containment, serves source/tests, HEAD body — `file.startsWith(ROOT)` hygiene; every file under web/ served; HEAD returns body. | DONE | S22 | allow-list `/`, `/index.html`, `/style.css`, `/app.js`, `/command.mjs`; everything else under `web/` 404; `assertContainedPath()` reused as defence... | - |
...
| U-73 | resolve_path CWD fallback contradicts CWD-independent contract — relative input missing next to conf but exists in CWD picked... | OPEN | - | not started; scoped as P1-43 in COMPILED_AUDIT.md §6 | P1-43: CLI resolve_path and batch+output semantics — NEW F:NF-16/17/19 — fable 5.1 low — WINNER. |
| U-74 | Batch + output + N>1 passes planner in merge shape, engine semantics undocumented — GS-201 refuses Batch WITHOUT output; WITH... | OPEN | - | not started; scoped as P1-43 in COMPILED_AUDIT.md §6 | P1-43: CLI resolve_path and batch+output semantics — NEW F:NF-16/17/19 — fable 5.1 low — WINNER. |
| U-75 | `-E` exposed but `--name` not, several engine options absent — without `--name` `-E` vs `-e` differs only if input already... | OPEN | - | not started; scoped as P3-12 in COMPILED_AUDIT.md §6 | P3-12: -E without --name and prefix docs — NEW F:NF-18 — fable 5.1 low — WINNER. |
| U-76 | Explode default prefix differs per surface — GUI: `<dir>/<stem>_frame`, CLI: `<basename>.NNN` in CWD when output cleared;... | OPEN | - | not started; scoped as P1-43 in COMPILED_AUDIT.md §6 | P1-43: CLI resolve_path and batch+output semantics — NEW F:NF-16/17/19 — fable 5.1 low — WINNER. |
<!-- END GENERATED -->
...
| W-04 | CLI driver + unit tests + integration smoke | DONE | S22 | `./build.sh` 308 checks, 0 failures; `scripts/smoke_cli.sh` 45/45 (adds U-53/U-60/U-61/U-62/DS-09 regressions on top of the earlier GS-207 + GS-203 coverage) | - |
...
| GS-210 | Strict build arguments, GUI build-tool selection and version integration | PARTIAL | S17 | Both entry points parse all args before writes/tools; unknown rc=2, help rc=0; 12 isolated parser cases pass and the current native build is 308/0. qmake-first dispatch and hardcoded .pro VERSION are unchanged | **P2-13** — Qt-equipped agent implements/tests CMake-first or CMake-only dispatch and version consistency |
...
| DS-09 | `threads < -1` is accepted with no warning and re-interpreted as auto | DONE | S22 | `src/core/Validate.h` + `web/validate.mjs` now warn on values below `-1`; `./build.sh` 308/308, `web/test/validate.test.mjs`, and `scripts/smoke_cli.sh` pin `threads = -7` | - |
