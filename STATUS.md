...
**Counts (generated - do not edit by hand):** 89 DONE · 9 PARTIAL · 48 OPEN · 0 UNTRIAGED · 146 total
...
| U-67 | `serveStatic()` served the whole `web/` tree — server source, test suite and docs were world-readable (narrowed by S21 measurement; two of the three original sub-claims did not survive it — see §2F F-10). | PARTIAL | S21 | REMAINING: the engine-gated `transport.test.mjs` no-regression re-run (its only static dependency is the readiness `GET /`, which is preserved and ... | P2-16: Web static hygiene and 413 mapping — NEW F:NF-10/11/12 — fable 5.1 low — WINNER. |
