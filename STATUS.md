...
| N-07 | Sweep **S4** detects only 5 hardcoded retired phrases (`demo only`, `not the product path`, `is a demo`, `demo/parity harness`, `not the product`), so it cannot catch a current-state doc asserting findings "stay `UNTRIAGED`" after a triage empties the register | OPEN | S16 | Measured S15: `sweep_stale.sh`'s `RETIRED=` is 5 literals. The S15 triage left 4 live claims of that shape and the sweep reported `5 passed, 0 failed` both before and after they were corrected by hand. S4 is a 5-phrase list, not a general reversal detector. | **P2-15** — Fail when a quoted UNTRIAGED count disagrees with STATUS.md's generated counts line, or restate S4 as a fixed phrase list |
