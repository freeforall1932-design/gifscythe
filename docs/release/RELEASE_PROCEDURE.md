...
./scripts/smoke_cli.sh        # CLI integration                -> 12/12
...
Expected counts as of S11 (2026-09-12): unit suite **293 checks, 0 failures**
(the runtime counter, not the 258 `CHECK(` source sites), GUI harness **317
runtime checks** *(measured in the S11 sandbox, which had Qt 6.4.2 — re-run it
on a Qt machine before trusting the number; the file now holds 245 `CHECK(`
source sites, which is a different quantity)*, smoke **12/12**, web **17 + 21 +
